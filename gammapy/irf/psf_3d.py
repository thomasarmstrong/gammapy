...
class PSF3D_update(object):
    ...
    offset_lo : `~astropy.units.Quantity`
        Offset angle bins lower edges (1-dim)
    offset_hi : `~astropy.units.Quantity`
        Offset angle bins upper edges (1-dim)
    scale : `~astropy.units.Quantity`
        Absolute scale of the 1st Gaussian
    sigma_1 : `~astropy.units.Quantity`
        model parameter
    ampl_2 : `~astropy.units.Quantity`
        model parameter
    sigma_2 : `~astropy.units.Quantity`
        model parameter
    ampl_3 : `~astropy.units.Quantity`
        model parameter
    sigma_3 : `~astropy.units.Quantity`
        model parameter
    """
    ...
    def __init__(self, energy_lo, energy_hi, offset_lo, offset_hi, scale,
                 sigma_1, ampl_2, sigma_2, ampl_3, sigma_3,
                 meta=None, interp_kwargs=None):
        if interp_kwargs is None:
            interp_kwargs = self.default_interp_kwargs
        axes = [
            BinnedDataAxis(energy_lo, energy_hi, interpolation_mode='log', name='energy'),
            BinnedDataAxis(offset_lo, offset_hi, interpolation_mode='linear', name='offset')
        ]
        self.scale = NDDataArray(axes=axes, data=scale,
                                 interp_kwargs=interp_kwargs)
        self.ampl_2 = NDDataArray(axes=axes, data=ampl_2,
                                  interp_kwargs=interp_kwargs)
        self.ampl_3 = NDDataArray(axes=axes, data=ampl_3,
                                  interp_kwargs=interp_kwargs)
        self.sigma_1 = NDDataArray(axes=axes, data=sigma_1,
                                   interp_kwargs=interp_kwargs)
        self.sigma_2 = NDDataArray(axes=axes, data=sigma_2,
                                   interp_kwargs=interp_kwargs)
        self.sigma_3 = NDDataArray(axes=axes, data=sigma_3,
                                   interp_kwargs=interp_kwargs)
    ...
    #     """
    #     # TODO: should call helper function here, not re-implement this!
    #     return 10 ** ((np.log10(self.data.axis('energy').hi / Quantity(1, self.data.axis('energy').hi.unit))
    #                    + np.log10(self.data.axis('energy').lo / Quantity(1, self.data.axis('energy').lo.unit)))
    #                   / 2) * Quantity(1, self.data.axis('energy').lo.unit)
    #
    # def rad_center(self): #TODO: same as a bove
    #     """Get centers of rad bins (`~astropy.coordinates.Angle` in deg).
    #     """
    #     return ((self.rad_hi + self.rad_lo) / 2).to('deg')
    ...
    def read(cls, filename, hdu='POINT SPREAD FUNCTION'):
        ...
        filename : str
            File name
        hdu : str
            Name of HDU name in fits file
        """
        filename = make_path(filename)
        hdulist = fits.open(str(filename))
        table = Table.read(hdulist, hdu=hdu)
        return cls.from_table(table)
    ...
    def from_table(cls, table):
        ...
        return cls(
            ...
            scale=table['SCALE'].quanitity[0],
            ampl_2=table['AMPL_2'].quanitity[0],
            ampl_3=table['AMPL_3'].quanitity[0],
            sigma_1=table['SIGMA_1'].quanitity[0],
            sigma_2=table['SIGMA_2'].quanitity[0],
            sigma_3=table['SIGMA_3'].quanitity[0],
        )
    ...
    def evaluate(self, energy=None, offset=None, rad=None, #TODO Can this just be replaced with the evaluate function?
                 interp_kwargs=None):
        """Interpolate the value of the `EnergyOffsetArray` at a given offset and energy.
    ...
    def to_energy_dependent_table_psf(self, theta=None, exposure=None):
        ...
        theta = theta or Angle(0, 'deg')
        energies = self.scale.axis('energy').nodes
        rad = self.rad_center()
        psf_value = self.evaluate(offset=theta).squeeze().T
